defmodule Aoc2023.Day21One do
  ...
  defp walk({x, y} = point, matrix, steps, h, w) do
    ...
    if steps > 0 and (is_nil(visited_at_step) or visited_at_step < steps) do
      ...
      |> Enum.each(fn {x_off, y_off} ->
        ...
        next = Map.get(matrix, {rem(n_x, h), rem(n_y, w)})
        if next == "." or next == "S", do: walk(next_coord, matrix, steps - 1, h, w)
      end)
    end
  end
